-- Fix Relevant Insights RLS Authentication Issue
...
-- Create simple, efficient policies using family_id
...
begin
    ...
    -- Check if the calling user belongs to this family
    if not exists (
        select 1 from profiles p
        where p.id::text = auth.uid()::text
...
-- Test query (run this manually to verify with a real guidance_id)
